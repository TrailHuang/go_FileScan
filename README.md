...
一个高性能的Go语言文件病毒扫描程序，支持实时监控目录、动态学习表匹配、黑样本隔离和多种扫描模式。
...
- ✅ **黑样本隔离** - 可选的黑样本隔离功能，自动移动病毒文件到隔离目录
- ✅ **高性能并发** - 支持多线程并发扫描
...
**隔离配置说明**:
- `enabled`: 是否启用隔离功能（默认 false）
- `path`: 隔离目录路径，启用时会自动创建
...
4. **隔离功能问题**
   - 检查隔离目录是否存在和可写
   - 确认配置文件中 quarantine.enabled 为 true
   - 检查文件系统权限
